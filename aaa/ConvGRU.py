...
class ConvGRUCell(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_size, bias):
        ...
        self.attention = AttentionBlock(200,200,200)
...
class ConvGru(nn.Module):
    def __init__(self, input_size,input_dim, hidden_dim, kernel_size, num_layers, batch_first=False, bias=True,
        ...
        self.c_attention = ChannelAttention(input_dim)
        self.attention = AttentionBlock(200, 200, 200)
        # 为了储存每一层的参数尺寸
        cell_list = []
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        for layer_idx in range(self.num_layers):
            ...
            #self.c_attention = ChannelAttention(layer_output.shape[2]).cuda()
            #layer_output = self.c_attention(layer_output)
        ...
        # 郭师兄在这里加了个线性层
        layer_output_list = self.cell_list[-1](self.cell_list[-2](layer_output_list[0]).reshape(b, seq_len, self.hidden_dim[-1], -1)).permute(0, 1, 3,2)
...
class CONVGRUNet(nn.Module):
    def __init__(self,input_size, input_dim, output_dim, hidden_dim, kernel_size, num_layers, batch_first=False, bias=False,
                 return_all_layers=False):
        super(CONVGRUNet, self).__init__()
        self.c_attention = ChannelAttention(input_dim)   # 加的通道注意力
        self.convgru = ConvGru(input_size,input_dim, hidden_dim, kernel_size, num_layers, batch_first=True, bias=True,
                                 return_all_layers=False)
        # self.fc = nn.Linear(input_size[0]*input_size[1]*hidden_dim[-1], output_dim)
        # self.fc = nn.Linear(768000, output_dim)
        self.fc = nn.Linear(10,output_dim)

    def forward(self, inputs):
        # pdb.set_trace()
        #inputs = self.c_attention(inputs)   # 通道注意力
        o1, h1 = self.convgru(inputs)
        # o1 = o1.reshape(o1.shape[0], o1.shape[1] * o1.shape[2] * o1.shape[3])
        out = o1.squeeze()
        # out = self.fc(o1)
        # pdb.set_trace()
        # o1 = o1[0]
        # o1 = o1.reshape(o1.shape[0], o1.shape[1] * o1.shape[2] * o1.shape[3] * o1.shape[4])
        # out = self.fc(o1)
        # pdb.set_trace()
        return out